...
function SquareDike(;
    ...
    return SquareDike(Cg, convert(GeoUnit, Angle), Wg, Hg, Lengthscale, BoundingBox, convert(GeoUnit, RotMat))
end
...
"""
function inside(p::Point{2, _T}, sill::SquareDike{2, _T}; rotate::Bool=true) where {_T}
    GeoParams.@unpack_val W, H, Center, RotMat = sill
    p_r = p - Center
    if rotate
...
"""
function inside(p::Point{3, _T}, sill::SquareDike{3, _T}; rotate::Bool=true) where {_T}
    GeoParams.@unpack_val W, H, Center, RotMat = sill
    p_r = p - Center
    if rotate
        p_r = rotate_point(p_r, RotMat)
